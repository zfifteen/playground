...
def run_experiment(max_prime: int = 10**9,
                  n_bins: int = 100,
                  use_cache: bool = True,
                  verbose: bool = True):
    ...
    plot_decay_trend(bin_analysis, regression, 
                    str(results_dir / 'decay_trend.png'), suffix)
    plot_log_gap_histogram(log_gaps, 
                          str(results_dir / 'log_gap_histogram.png'), 
                          n_bins=100, title_suffix=suffix)
    plot_qq_lognormal(log_gaps, 
                     str(results_dir / 'qq_plot_lognormal.png'), suffix)
    plot_acf(acf_pacf, 
            str(results_dir / 'acf.png'), suffix)
    plot_pacf(acf_pacf, 
             str(results_dir / 'pacf.png'), suffix)
    plot_log_prime_vs_log_gap(log_primes, log_gaps, 
                             str(results_dir / 'log_prime_vs_log_gap.png'), suffix)
    plot_box_plot_per_bin(log_gaps, bin_analysis['assignments'][:-1], 
                         str(results_dir / 'box_plot_per_bin.png'), 
                         n_bins=n_bins, title_suffix=suffix)
    plot_cdf(log_gaps, 
            str(results_dir / 'cdf.png'), suffix)
    plot_kde(log_gaps, 
            str(results_dir / 'kde.png'), suffix)
    plot_regression_residuals(bin_analysis, regression, 
                             str(results_dir / 'regression_residuals.png'), suffix)
    plot_log_gap_vs_regular_gap(regular_gaps, log_gaps, 
                               str(results_dir / 'log_gap_vs_regular_gap.png'), suffix)
    plot_prime_density(log_primes, 
                      str(results_dir / 'prime_density.png'), suffix)
    
    # Step 6: Generate 3D plots
    if verbose:
        print("[6/8] Generating 5 3D plots...")
    
    plot_scatter_3d(log_primes, log_gaps, 
                   str(results_dir / 'scatter_3d.png'), 
                   sample_size=10000, title_suffix=suffix)
    plot_surface_3d(log_primes, log_gaps, 
                   str(results_dir / 'surface_3d.png'), 
                   n_bins_x=50, n_bins_y=50, title_suffix=suffix)
    plot_contour_3d(log_gaps, 
                   str(results_dir / 'contour_3d.png'), 
                   max_lag=50, n_scales=5, title_suffix=suffix)
    plot_wireframe_3d(bin_analysis, 
                     str(results_dir / 'wireframe_3d.png'), suffix)
    plot_bar_3d(bin_analysis, 
               str(results_dir / 'bar_3d.png'), 
               n_groups=10, title_suffix=suffix)
    ...
    results = {
        ...
        'autocorrelation': {
            ...
            'acf_pacf_summary': {
                'significant_lags': make_serializable(acf_pacf.get('significant_lags', [])),
                'nlags': acf_pacf.get('nlags', 50)
            }
        },
        ...
        'checks': {
            ...
            'f4_falsified': ljung_box.get('f4_falsified', False),
    ...
    if verbose:
        ...
        print()
        print("Key Findings:")
        print(f"  - Regression slope: {regression['slope']:.4e} (p={regression['p_value']:.2e})")
...
def parse_arguments():
    ...
    parser.add_argument('--max-prime', type=float, default=1e9,

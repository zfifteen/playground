...
def kolmogorov_smirnov_tests(log_gaps: np.ndarray) -> Dict:
    ...
    results['lognormal'] = {
        'params': {'shape': lognorm_params[0], 'loc': lognorm_params[1], 'scale': lognorm_params[2]},
    ...
    results['gamma'] = {
        'params': {'alpha': gamma_params[0], 'loc': gamma_params[1], 'scale': gamma_params[2]},
    ...
    results['weibull'] = {
        'params': {'shape': weibull_params[0], 'loc': weibull_params[1], 'scale': weibull_params[2]},
    ...
    f2_falsified = results['normal']['ks_statistic'] < results['lognormal']['ks_statistic']
...
def ljung_box_test(log_gaps: np.ndarray, max_lag: int = 50) -> Dict:
    """
    ...
    lb_result = acorr_ljungbox(log_gaps, lags=max_lag, return_df=False)
    ...
    if isinstance(lb_result, tuple):
        ...
        # Assume DataFrame
        lb_stats = lb_result['lb_stat'].values if hasattr(lb_result, 'lb_stat') else lb_result.iloc[:, 0].values
        p_values = lb_result['lb_pvalue'].values if hasattr(lb_result, 'lb_pvalue') else lb_result.iloc[:, 1].values
    ...
    return {
        'lb_stats': lb_stats.tolist() if hasattr(lb_stats, 'tolist') else list(lb_stats),
